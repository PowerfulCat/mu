...
from mu.interface.panes import CHARTS, \
    MicroPythonDeviceFileList, PANE_ZOOM_SIZES
...
class SeeedFileSystemPane(QFrame):
    ...
    def __init__(self, home):
        ...
        layout = QGridLayout()
        ardupy_label = QLabel()
        ardupy_label.setText('Files on your device:')
        local_label = QLabel()
        local_label.setText('Files on your computer:')
        self.setLayout(layout)
        self.ardupy_label = ardupy_label
        self.local_label = local_label
        self.microbit_fs = microbit_fs
        self.local_fs = local_fs
        self.set_font_size()
        layout.addWidget(ardupy_label, 0, 0)
    ...
    def on_ls(self, ardupy_files):
        """
        Displays a list of the files on the ardupy.

        Since listing files is always the final event in any interaction
        between Mu and the ardupy, this enables the controls again for
        further interactions to take place.
        """
        self.microbit_fs.clear()
        self.local_fs.clear()

        for f in ardupy_files:
    ...
    def set_font_size(self, new_size=DEFAULT_FONT_SIZE):
        ...
        self.ardupy_label.setFont(self.font)
...
class SeeedMode(MicroPythonMode):
    ...
    def add_fs(self):
        ...
        self.view.default_panel = SeeedFileSystemPane
